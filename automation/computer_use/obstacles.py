...
_REDACT_RE = re.compile(r"(?i)(bearer\s+)[A-Za-z0-9._-]+|(?i)(api[_ -]?key\s*[=:]\s*)\S+|(?i)(token\s*[=:]\s*)\S+")
...
def _redact(value: Any) -> Any:
    if isinstance(value, str):
        ...
        return _REDACT_RE.sub(lambda match: (match.group(1) if match.group(1) else "") + "[redacted]", text)
