...
class CapabilitySpec:
    """Document a capability and the authorization level required to use it."""
...
class LocalAccessBroker:
    ...
    The broker intentionally implements only read/observe capabilities. New
    write, command, desktop, credential, or financial capabilities must be
    added behind the central authorization boundary rather than by extending
    this class with ad-hoc shell access.
    """
    ...
    def system_info(self) -> dict[str, Any]:
        roots = [str(root) for root in self.allowed_roots or ()]
        return {
    ...
    def _resolve(self, requested: str) -> Path:
        ...
        if value:
            ...
            if raw.is_absolute():
                ...
                candidates.extend((root / raw for root in self.allowed_roots or ()))
        else:
            candidates.extend(self.allowed_roots or ())
        for candidate in candidates:
            try:
                resolved = candidate.resolve()
            except OSError as exc:
                raise LocalAccessError("unable to resolve local path") from exc
            if self._under_allowed_root(resolved):
                return resolved
    ...
    def _is_sensitive(self, path: Path) -> bool:
        ...
        return bool(_SECRET_NAME_RE.search(normalized)) or any(part == ".git" for part in path.parts)
