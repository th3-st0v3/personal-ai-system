...
test('legacy watchdog declares a version and localhost bridge contract', () => {
    assert.match(source, /@version\s+1\.0\.0/);
    assert.match(source, /127\.0\.0\.1:8765/);
    assert.match(source, /\/browser\/observation/);
    assert.match(source, /Do not enable alongside the native PASI Chromium controller/);
});

test('legacy watchdog distinguishes context exhaustion from provider usage limits', () => {
...
test('legacy watchdog reports authentication challenges and Thinking state without changing the page', () => {
...
test('legacy watchdog samples runtime health on a bounded cadence', () => {
