...
class BridgeState:
    ...
    def get_status(self) -> dict[str, Any]:
        with self.lock:
            ...
            active_statuses = {"queued", "claimed", "generating"}
            queue_size = sum(1 for item in queue if item.get("status") in active_statuses)
    ...
    def _new_operation_id() -> str:
        ...
        return f"op-{time.time_ns()}-{secrets.token_hex(4)}"
...
class BridgeRequestHandler(BaseHTTPRequestHandler):
    """Small localhost HTTP API consumed by the Tampermonkey ChatGPT controller."""
    ...
    def _set_headers(self, status: int = HTTPStatus.OK) -> None:
        ...
        allowed_origins = {"https://chatgpt.com", "https://www.chatgpt.com"}
    ...
    def do_GET(self) -> None:
        ...
        if path == "/health":
            self._send_json({"status": "ok", "service": "personal-ai-system-chatgpt-bridge"})
            return
        if path == "/status":
            self._send_json(self.bridge_state.get_status())
            return
        if path == "/browser/observation":
            self._send_json({"observation": self.bridge_state.get_browser_observation()})
            return
        if path == "/operation":
        ...
        if path == "/next-operation":
            self._send_json({"operation": self.bridge_state.claim_next_operation()})
            return
    ...
    def _browser_observation(self, payload: dict[str, Any]) -> None:
        observation = payload.get("observation")
        if not isinstance(observation, dict):
            self._send_json({"error": "observation must be an object."}, HTTPStatus.BAD_REQUEST)
            return
        schema_version = observation.get("schema_version")
        if not isinstance(schema_version, str) or not schema_version.strip():
            self._send_json({"error": "observation.schema_version is required."}, HTTPStatus.BAD_REQUEST)
            return
        self._send_json({"observation": self.bridge_state.save_browser_observation(observation)}, HTTPStatus.CREATED)

    def _queue(self, payload: dict[str, Any]) -> None:
        operation_type = payload.get("operation_type")
        prompt = payload.get("prompt")
        if not isinstance(operation_type, str) or not operation_type.strip():
            self._send_json({"error": "operation_type is required."}, HTTPStatus.BAD_REQUEST)
            return
        if not isinstance(prompt, str):
            self._send_json({"error": "prompt must be a string."}, HTTPStatus.BAD_REQUEST)
            return
        if operation_type != "new_chat" and not prompt.strip():
            self._send_json({"error": "prompt is required for this operation type."}, HTTPStatus.BAD_REQUEST)
            return
        operation = self.bridge_state.queue_operation(operation_type=operation_type, prompt=prompt)
        self._send_json({"operation": operation.to_dict()}, HTTPStatus.CREATED)
    ...
    def _heartbeat(self, payload: dict[str, Any]) -> None:
        ...
        if not isinstance(operation_id, str):
            self._send_json({"error": "operation_id is required."}, HTTPStatus.BAD_REQUEST)
            return
        operation = self.bridge_state.heartbeat(operation_id)
        if operation is None:
            self._send_json({"error": "Operation not found."}, HTTPStatus.NOT_FOUND)
            return
        self._send_json({"operation": operation})
    ...
    def _finished(self, payload: dict[str, Any]) -> None:
        ...
        if chat_url is not None and not isinstance(chat_url, str):
            ...
            return
        operation = self.bridge_state.complete_operation(operation_id=operation_id, chat_url=chat_url)
        if operation is None:
            self._send_json({"error": "Operation not found."}, HTTPStatus.NOT_FOUND)
            return
        self._send_json({"operation": operation})

    def _failed(self, payload: dict[str, Any]) -> None:
        operation_id = payload.get("operation_id")
        ...
        if not isinstance(error, str) or not error.strip():
            self._send_json({"error": "error is required."}, HTTPStatus.BAD_REQUEST)
            return
        operation = self.bridge_state.fail_operation(operation_id=operation_id, error=error)
        if operation is None:
            self._send_json({"error": "Operation not found."}, HTTPStatus.NOT_FOUND)
            return
        self._send_json({"operation": operation})
    ...
    def log_message(self, format_string: str, *args: Any) -> None:
        print("[Bridge] " + format_string % args)
...
class ChatGPTBridge:
    def __init__(self, host: str = HOST, port: int = PORT):
        ...
        self.host = host
        self.port = port
        self.bridge_state = BridgeState(StateManager(CONFIG.ai_dir))
        self.server = BridgeHTTPServer((self.host, self.port), BridgeRequestHandler)
        self.server.bridge_state = self.bridge_state

    def run(self) -> None:
        print()
        print("=== CHATGPT LOCAL BRIDGE ===")
        print()
        ...
        print("  GET  /operation?operation_id=<id>")
        print("  POST /queue")
        print("  POST /chat/heartbeat")
        print("  POST /chat/finished")
