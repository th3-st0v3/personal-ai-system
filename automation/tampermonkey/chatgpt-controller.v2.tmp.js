...
(function () {
    ...
    function bridgeRequest(path, options) {
        ...
        return new Promise(function (resolve, reject) {
            GM_xmlhttpRequest({
                ...
                onerror: function () {
                    reject(new Error('Bridge request failed: ' + path));
                },
                ontimeout: function () {
                    reject(new Error('Bridge request timed out: ' + path));
                }
            });
        });
    }
    ...
    function findNewChat() {
        ...
        for (i = 0; i < elements.length; i += 1) {
            ...
            if (normalize(getLabel(element)) === 'new chat') {
                return element;
            }
        }
    ...
    function findSendButton() {
        ...
        for (i = 0; i < elements.length; i += 1) {
            ...
            if (label.indexOf('send prompt') !== -1 || label.indexOf('send message') !== -1 || label === 'send') {
                return elements[i];
            }
        }
    ...
    function getLabel(element) {
        ...
        return [aria, title, labelledText, element.textContent || '']
            .filter(Boolean)
            .join(' ')
            .trim();
    }
    ...
    async function reportFinished(operationId, responseObserved) {
        ...
        if (!response.ok) {
            throw new Error('Bridge completion failed: HTTP ' + response.status);
        }
    }
