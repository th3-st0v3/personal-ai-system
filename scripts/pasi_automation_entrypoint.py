...
def enrich_task(task: str) -> str:
    ...
    return (
        ...
        + "PULL SHARK OBJECTIVE (SECONDARY): Deliberately work toward GitHub Pull Shark Gold (1,024 eligible merged pull requests) while keeping the current PASI engineering objective first. Prefer independently reviewable, independently valuable atomic changes: one verified fix, test, documentation improvement, refactor, or narrowly scoped feature per task/PR. When a larger task naturally contains multiple independent changes, finish the highest-value atomic slice now and leave the next slice as the explicit next task so it can become its own PR. Never create empty, cosmetic, duplicate, no-op, or intentionally fragmented changes solely to increase the PR count. Never split a cohesive safety-critical change if doing so would reduce correctness or verification quality. Standard-risk verified PRs may use the existing auto-merge path; high-risk controller, browser, security-boundary, provider-routing, and workflow changes remain subject to their existing human-review gate.\n"
