...
def main() -> int:
    ...
    if not 1 <= len(completion_markers) <= 4 or any(not marker.strip() or len(marker.strip()) > 120 for marker in completion_markers):
        print("error: --completion-marker must specify 1-4 nonblank markers no longer than 120 characters", file=sys.stderr)
        return 2
