...
def main() -> int:
    ...
    search_parser = subparsers.add_parser("search", help="Search approved workspace text files without exposing sensitive files.")
    ...
    if args.command == "capabilities":
        payload = {
            "status": "ok",
            "capabilities": gateway.broker.capabilities()
            + [{"name": "computer.files.search", "risk": "safe", "description": "Search bounded text content inside explicitly approved workspace roots."}],
        }
        ...
        payload = gateway.dispatch({
            ...
            "capability": "computer.files.search",
            "parameters": {"query": args.query, "limit": args.limit},
        })
    else:
        payload = gateway.dispatch({
            "request_id": "cli-read",
