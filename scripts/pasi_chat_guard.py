...
def execute_computer_requests(response: str, repo_root: Path) -> list[dict[str, Any]]:
    ...
    for request in requests:
        results.append(gateway.dispatch(request))
    return results
...
For a needed read/inspection, return exactly one fenced request section like this:
{REQUEST_BEGIN}
{{"request_id":"read-1","capability":"computer.files.read","parameters":{{"path":"relative/path.txt","max_chars":20000}}}}
{REQUEST_END}
...
Each request is executed by PASI, not by you. Writes, commands, desktop control, credential access, and financial execution are not available through this protocol.
...
def run_guarded(command: list[str], *, timeout: float, bridge_poll_seconds: float = POLL_SECONDS) -> int:
    code, classification, combined = run_child(command, timeout=timeout, bridge_poll_seconds=bridge_poll_seconds)
    if combined:
        print(combined, end="")
