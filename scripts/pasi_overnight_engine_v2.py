...
def continuation_directive(state: OvernightState, _task: str | None = None) -> str:
    """Return only task-local continuation guidance.

    The durable scheduler keeps the full roadmap and recent-task history in
    machine-readable state. Those lists are intentionally not copied into every
    model prompt: stale history increases prompt size and can distract the
    implementation agent from the current objective.
    """
