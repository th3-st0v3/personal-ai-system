...
# Discover source/test files from the working tree while excluding dependency,
# generated, cache, and runtime directories. This keeps new files covered
# without requiring this script to be edited when the repository grows.
pruned_dirs=(
    ./.git
    ./.venv
    ./venv
...
mapfile -d '' JAVASCRIPT_FILES < <(
    "${find_expr[@]}" -name '*.js' -print0 | sort -z
)
...
if ((${#NON_SRC_PYTHON_TEST_FILES[@]})); then
    run_check "All discovered non-src Python tests" python -m pytest -q "${NON_SRC_PYTHON_TEST_FILES[@]}"
fi

run_check "Static type check" pyright "${PYTHON_FILES[@]}"
run_check "Markdown lint" npx --yes markdownlint-cli2@latest '**/*.md' '#node_modules' '#**/.venv/**' '#**/venv/**' '#**/env/**' '#**/.git/**' '#**/.pytest_cache/**' '#**/.runtime/**' '#**/runtime/**' '#**/dist/**' '#**/build/**' '#**/coverage/**' '#**/htmlcov/**'
