(() => {
  ...
  const currentChat=async()=>state.chatId?api(`/api/chats/${state.chatId}`):null;
  ...
  const bind=()=>{
    ...
    if(title&&!title.dataset.renameBound){title.dataset.renameBound='1';title.tabIndex=0;title.title='Rename chat';title.addEventListener('click',(e)=>{e.preventDefault();runAction('rename').catch((err)=>window.showError?.(err));});title.addEventListener('keydown',(e)=>{if(e.key==='Enter'){e.preventDefault();runAction('rename').catch((err)=>window.showError?.(err));}});}
  ...
  document.addEventListener('click',(event)=>{const action=event.target.closest?.('[data-chat-action]');if(action){event.preventDefault();event.stopPropagation();runAction(action.dataset.chatAction).catch((err)=>window.showError?.(err)).finally(()=>document.querySelector('.final-chat-menu')?.remove());return;}const menu=document.querySelector('.final-chat-menu');if(menu&&!menu.contains(event.target)&&!event.target.closest('#chat-title-menu'))menu.remove();});
  document.addEventListener('keydown',(event)=>{if(event.key==='Escape')document.querySelector('.final-chat-menu')?.remove();if(!event.target.matches?.('input,textarea,[contenteditable=true]')&&document.querySelector('.final-chat-menu')){if(event.key.toLowerCase()==='p')runAction('pin').catch((err)=>window.showError?.(err));if(event.key.toLowerCase()==='r')runAction('rename').catch((err)=>window.showError?.(err));if(event.key.toLowerCase()==='d')runAction('delete').catch((err)=>window.showError?.(err));}});
  document.addEventListener('click',(event)=>{const auth=event.target.closest?.('#login-button,#signup-button');if(!auth)return;event.preventDefault();event.stopImmediatePropagation();window.__pasOpenAuth?.(auth.id==='signup-button'?'signup':'login');},true);
  new MutationObserver(bind).observe(document.body,{childList:true,subtree:true});window.addEventListener('load',bind,{once:true});
})();
