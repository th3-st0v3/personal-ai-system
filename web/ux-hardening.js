(() => {
  ...
  const stop = (selector, handler) => document.addEventListener('click', event => { const target=event.target.closest(selector); if(!target)return; event.preventDefault(); event.stopImmediatePropagation(); try { handler(target,event); } catch(error) { notify(error.message); } }, true);
  ...
  document.addEventListener('click', event => { if(!event.target.closest('.ux-context-menu'))document.querySelector('.ux-context-menu')?.remove(); const back=event.target.closest('[data-back-chat]'); if(back){event.preventDefault();openChat(state.chatId)} if(event.target.closest('#logout-action')){event.preventDefault();(async()=>{try{await send('/api/auth/logout',{});state.user=null;closeModal();await refreshAuth();notify('Logged out.','ok')}catch(error){notify(error.message)}})()} }, true);
