(() => {
  ...
  const renderMessages = (messages, toolEvents = []) => {
    ...
    box.innerHTML = messages.map((m) => {
      if (m.role === 'user') return `<article class="message user"><div class="user-bubble">${escHtml(m.content)}</div><div class="message-actions"><button data-msg-action="copy" data-msg-id="${m.id}">Copy</button><button data-msg-action="share" data-msg-id="${m.id}">Share</button><button data-msg-action="edit" data-msg-id="${m.id}">Edit</button></div></article>`;
  ...
  const modelFor = () => {
    const value = $('ai-mode')?.value || 'auto';
    return ({
      auto: 'profile:auto',
  ...
  const saveTitle = async () => {
    ...
    const value = prompt('Chat name', $('chat-title')?.textContent || 'New chat');
    if (value == null) return;
    await patch(`/api/chats/${state.chatId}`, { title: value });
    $('chat-title').textContent = value.trim();
    await window.loadChats?.();
  };
  const chatMenu = (x, y) => {
    document.querySelector('.chat-menu')?.remove();
    const menu = document.createElement('div');
    menu.className = 'chat-menu';
    menu.style.left = `${Math.max(8, Math.min(x, innerWidth - 250))}px`;
    menu.style.top = `${Math.max(8, Math.min(y, innerHeight - 280))}px`;
    const actions = [['Rename','rename'],['Pin','pin'],['Move to project','move'],['Share chat','share'],['Delete','delete']];
    actions.forEach(([label, action]) => { const b=document.createElement('button'); b.textContent=label; b.dataset.chatAction=action; menu.appendChild(b); });
  ...
  const decorate = () => {
    $('brand-home')?.remove();
    $('format-toggle')?.remove();
    $('format-toolbar')?.remove();
    $('chat-context') && ($('chat-context').textContent='');
    const title=$('chat-title');
    if(title && !title.dataset.decorated){ title.dataset.decorated='1'; title.title='Click to rename'; title.addEventListener('click',saveTitle); }
    const oldHeader=$('chat-header');
    if(oldHeader && !oldHeader.querySelector('.chat-title-menu')){ const button=document.createElement('button');button.className='chat-title-menu';button.textContent='⌄';button.title='Chat actions';button.onclick=(e)=>{e.stopPropagation();chatMenu(e.clientX,e.clientY)};oldHeader.querySelector('.header-actions')?.prepend(button); }
    const sidebar=$('sidebar');
    if(sidebar && !sidebar.querySelector('.account-dock')){const dock=document.createElement('button');dock.className='account-dock';dock.onclick=()=>openSettingsPage();dock.innerHTML='<span class="account-avatar">?</span><span class="account-name">Account</span>';sidebar.appendChild(dock);}
  ...
  document.addEventListener('click', async (event) => {
    const target=event.target.closest?.('[data-tools-toggle], [data-msg-action], [data-chat-action], .chat-row, [data-view="projects"], [data-view="settings"], [data-view="chat"], #new-chat, #new-chat-header, #calculations-toggle, [data-major-page]');
    if(!target)return;
    event.preventDefault(); event.stopImmediatePropagation();
    try {
      if(target.matches('[data-tools-toggle]')) { const log=target.nextElementSibling; log.hidden=!log.hidden; return; }
      if(target.matches('[data-msg-action]')) { const action=target.dataset.msgAction; const article=target.closest('.message'); const text=article?.querySelector('.content,.user-bubble')?.textContent||''; if(action==='copy'){await navigator.clipboard.writeText(text)} else if(action==='share'){await navigator.share?.({text})} else if(action==='edit'){ $('chat-input').value=text; $('chat-input').focus(); } else if(action==='retry'){ $('chat-input').value=text; await $('chat-form').requestSubmit(); } else if(action==='branch'){ const r=await send('/api/chats',{project_id:state.projectId,title:'Branch'}); state.chatId=r.id; $('chat-input').value=text; await $('chat-form').requestSubmit(); } return; }
      ...
      if(target.matches('.chat-row')) { state.chatId=Number(target.dataset.chatId); await openChat(state.chatId); renderMessages((await api(`/api/chats/${state.chatId}`)).messages); return; }
  ...
  document.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      const result=await send(`/api/chats/${state.chatId}/messages`,{content,mode:$('ai-mode')?.value==='free'?'auto':$('ai-mode')?.value==='auto'?'auto':'auto',model:modelFor()});
      renderMessages(result.messages,result.tool_events||[]); if(result.title)$('chat-title').textContent=result.title; await loadChats();
    } catch(error){notify(error.message)} finally { if($('send-chat'))$('send-chat').disabled=false; }
  }, true);
  document.addEventListener('keydown', (event) => {
    const typing=event.target.matches?.('input,textarea,[contenteditable="true"]');
    if((event.ctrlKey||event.metaKey) && ['o','k'].includes(event.key.toLowerCase()) && !typing){event.preventDefault();event.stopImmediatePropagation();draft();}
    if(event.key==='Escape'){document.querySelector('.chat-menu')?.remove();}
    if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='/'&&!typing){event.preventDefault();openSettingsPage();}
    if(event.key==='r'&&!typing&&$('chat-title')?.matches(':focus'))saveTitle();
  }, true);
  ...
  const observer=new MutationObserver(decorate); observer.observe(document.body,{subtree:true,childList:true});
  window.addEventListener('load',decorate,{once:true});
  window.__pasRenderMessages=renderMessages;
  window.__pasDraftChat=draft;
})();
