(() => {
  ...
  const open=(kind)=>{
    ...
    $('auth-ui-form').onsubmit=async(event)=>{event.preventDefault();try{const values=Object.fromEntries(new FormData(event.target));const result=await send(signup?'/api/auth/signup':'/api/auth/login',values);state.user=result.user;closeModal();await refreshAuth();}catch(error){$('auth-ui-form').insertAdjacentHTML('afterend',`<div class="error">${esc(error.message||error)}</div>`);}};
    $('auth-ui-form').querySelector('input')?.focus();
  };
  window.__pasOpenAuth=open;
  window.addEventListener('click',(event)=>{const button=event.target.closest?.('#login-button,#signup-button');if(!button)return;event.preventDefault();event.stopImmediatePropagation();open(button.id==='signup-button'?'signup':'login');},true);
})();
