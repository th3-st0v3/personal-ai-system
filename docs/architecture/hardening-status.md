...
- Pyright now excludes generated/hidden environment directories without excluding `src`, `scripts`, or tests. Dynamic JSON values are narrowed at the HTTP boundary rather than suppressing type diagnostics globally.
...
2. The web API still needs a complete canonical request-schema layer so every endpoint validates the same way before domain calls. The current boundary performs runtime checks in services and is typed as dynamic JSON at the edge; this is transitional rather than the final contract.
...
6. A historical standalone workspace abstraction was intentionally removed because it duplicated `WorkspaceApplication`. That deleted module was 552 lines and included conceptual `link`, `dataset`, and `resource` item types in addition to folder/note/file behavior. The current workspace intentionally supports folder/note/file only. The broader item model is therefore a **future capability to restore behind an extensible item contract**, not something that should be silently treated as already implemented.

These distinctions are tracked as technical debt or future capability rather than ignored failures. No capability was deleted merely to make the repository appear cleaner.

## Historical consolidation audit

The major historical deletion identified in the architecture review is commit `78ad17182719a3cec88d51ef4032684269878ed2`, which removed the 552-line standalone workspace abstraction in favor of extending the existing application boundary. The intent was to eliminate two competing workspace implementations. The deleted `link`, `dataset`, and `resource` item concepts were not migrated into the current public item contract and must not be presented as implemented until they are restored deliberately.

The authoritative long-term requirements in `docs/requirements/SYSTEM-REQUIREMENTS.md` and the Engineering AI OS architecture remain broader than the beta implementation. They continue to govern future learning, research, multi-agent, engineering-runtime, geometry, simulation, V&V, heterogeneous-compute, and advanced UI work.
