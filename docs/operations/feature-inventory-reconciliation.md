...
- **Repository-wide branch hygiene** — a single branch-hygiene workflow observes all branch/PR lifecycle changes, reuses completed validation rather than rerunning the same head, and performs only bounded safe branch/PR reconciliation from canonical `main`.
