...
2. The harness waits until its exact prompt operation is claimed/generating **and the M2 manual-reload gate is armed**. The browser has already captured the single response and its timing evidence, but deliberately withholds terminal acknowledgement so the same operation remains recoverable.
3. It pauses for one manual action: reload the **exact ChatGPT conversation tab carrying that operation**, or close it and manually reopen the **same conversation URL** before continuing. No replacement conversation, human message, or second prompt is allowed. PASI does not automatically reopen a browser tab that the operator fully closed.
4. Before any process recovery continues, it requires a **fresh** native browser-health observation (at most 30 seconds old) with the expected controller version, extension manifest version, native-controller state, composer, conversation signature, and the **same conversation URL**. The same operation must still be non-terminal and its manual-reload gate must remain armed and unreleased. A stale observation, different conversation, or early terminal completion fails at the manual-reload checkpoint.
5. It terminates only the managed bridge PID tree, waits for the bridge to become unavailable, restarts the same bridge runtime, and verifies the original operation ID still exists and is non-terminal.
6. It terminates only the managed runner PID tree. The test sets `PASI_SUPERVISOR_MAX_RESTARTS=0` and requires runner-log evidence that the supervisor exhausted that zero-restart budget, so the test cannot pass merely because a supervisor happened to disappear.
7. It resumes with `bash scripts/start_pasi_168h.sh --resume --no-push` using the exact persisted branch/worktree identity. Because the launcher detaches the supervisor, the acceptance reads the appended `runner.log` evidence rather than trusting launcher stdout.
8. After explicit resume evidence is recorded, the harness releases the manual-reload gate for the **original operation ID only**. The reopened browser tab then completes that held response locally; no second prompt is injected.
9. Resume evidence must explicitly say `Resuming persisted ChatGPT operation: <original-id>`; a new `Retry prompt operation` for that M2 task, or more than one queue operation bound to the unique marker, is a failure.
10. Final evidence must show the same operation ID completed, the same ChatGPT conversation URL, the required unique marker bound to exactly one operation, durable `user_messages_added: 1`, verified acknowledgement, and an exact +1 user/+1 assistant conversation-signature progression from the pre-operation baseline.
