...
4. Before any process recovery continues, it requires a **fresh** native browser-health observation (at most 30 seconds old) with the expected controller version, extension manifest version, native-controller state, composer, conversation signature, and the **same conversation URL**. A stale observation or a different conversation fails at the manual-reload checkpoint.
...
8. Resume evidence must explicitly say `Resuming persisted ChatGPT operation: <original-id>`; a new `Retry prompt operation` for that M2 task, or more than one queue operation bound to the unique marker, is a failure.
9. Final evidence must show the same operation ID completed, the same ChatGPT conversation URL, the required unique marker bound to exactly one operation, durable `user_messages_added: 1`, verified acknowledgement, and an exact +1 user/+1 assistant conversation-signature progression from the pre-operation baseline.
