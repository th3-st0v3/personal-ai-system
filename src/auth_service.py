...
def _now() -> float: return datetime.now(timezone.utc).timestamp()
...
def _require_credentials(email: str, password: str) -> tuple[str, str]:
    ...
    if "@" not in email or len(email) < 5: raise ValueError("Enter a valid email address.")
...
def signup(connection: sqlite3.Connection, email: str, password: str, display_name: str | None = None) -> dict[str, object]:
    ...
    email, password = _require_credentials(email, password); name=(display_name or email.split("@",1)[0]).strip() or "User"; salt=secrets.token_bytes(16)
    try:
        cursor=connection.execute("INSERT INTO users(email,display_name,password_salt,password_hash) VALUES(?,?,?,?)",(email,name[:80],salt.hex(),_hash_password(password,salt))); connection.commit()
    except sqlite3.IntegrityError as exc: raise ValueError("An account with that email already exists.") from exc
...
def login(connection: sqlite3.Connection, email: str, password: str) -> tuple[str, dict[str, object]]:
    initialize(connection); email,password=_require_credentials(email,password)
    row=connection.execute("SELECT id,email,display_name,password_salt,password_hash FROM users WHERE email=?",(email,)).fetchone()
    if row is None or not hmac.compare_digest(_hash_password(password,bytes.fromhex(row[3])),row[4]): raise ValueError("Email or password is incorrect.")
    token=secrets.token_urlsafe(32); _SESSIONS[token]=(int(row[0]),_now()+_SESSION_TTL_SECONDS)
...
def current_user(connection: sqlite3.Connection, token: str | None) -> dict[str, object] | None:
    ...
    if expiry<=_now(): _SESSIONS.pop(token,None); return None
    row=connection.execute("SELECT id,email,display_name,created_at FROM users WHERE id=?",(user_id,)).fetchone()
    if row is None: _SESSIONS.pop(token,None); return None
