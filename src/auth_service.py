...
def login(connection: sqlite3.Connection, email: str, password: str) -> tuple[str, dict[str, object]]:
    initialize(connection)
    email, password = _require_credentials(email, password)
    row = connection.execute("SELECT id,email,name,display_name,password_salt,password_hash FROM users WHERE email=?", (email,)).fetchone()
    if row is None:
...
def current_user(connection: sqlite3.Connection, token: str | None) -> dict[str, object] | None:
    ...
    row=connection.execute("SELECT id,email,name,display_name,created_at FROM users WHERE id=?",(user_id,)).fetchone()
    if row is None: _SESSIONS.pop(token,None); return None
    return {"id":row[0],"email":row[1],"display_name":row[3] or row[2] or "User","created_at":row[4]}
