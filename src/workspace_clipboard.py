...
def _copy_item(storage_root, project_id: int, kind: workspace_browser.ItemKind, item_id: int, target_folder_id: int | None) -> int:
    if kind == "folder":
        ...
        new_folder = workspace_storage.create_folder(project_id, _unique_name(project_id, target_folder_id, str(source[3])), target_folder_id)
        if new_folder is None: raise RuntimeError("Database did not return a new folder ID.")
        for child in workspace_browser.list_children(project_id, item_id, sort="a_z"): _copy_item(storage_root, project_id, child.kind, child.id, int(new_folder))
        return int(new_folder)
    if kind == "note":
...
def copy_selection(storage_root, project_id: int, selection: Iterable[tuple[workspace_browser.ItemKind, int]]) -> list[dict[str, Any]]:
    selected: set[tuple[workspace_browser.ItemKind, int]] = set()
    for kind, item_id in selection:
        selected.add((cast(workspace_browser.ItemKind, kind), int(item_id)))
    for kind, item_id in selected:
        if workspace_browser._item_project(kind, item_id) != project_id: raise ValueError("Selection contains an item from another project.")
...
def paste_selection(storage_root, project_id: int, target_folder_id: int | None, clipboard: Iterable[dict[str, Any]]) -> list[int]:
    ...
    typed_items: list[tuple[workspace_browser.ItemKind, int, dict[str, Any]]] = []
    for item in clipboard_items:
        ...
        typed_items.append((kind, item_id, item))
    created: list[int] = []
    for kind, item_id, item in typed_items:
