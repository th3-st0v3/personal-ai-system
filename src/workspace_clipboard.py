...
def _copy_item(storage_root, project_id: int, kind: workspace_browser.ItemKind, item_id: int, target_folder_id: int | None) -> int:
    if kind == "folder":
        ...
        if source is None or int(source[1]) != project_id:
        ...
        new_name = _unique_name(project_id, target_folder_id, str(source[3]))
        new_folder = workspace_storage.create_folder(project_id, new_name, target_folder_id)
        if new_folder is None:
            raise RuntimeError("Database did not return a new folder ID.")
...
def paste_selection(storage_root, project_id: int, target_folder_id: int | None, clipboard: Iterable[dict[str, Any]]) -> list[int]:
    ...
    for item in clipboard_items:
        ...
        if kind_raw not in {"folder", "note", "file"}:
            raise ValueError(f"Unsupported clipboard item kind '{kind_raw}'.")
        kind = cast(workspace_browser.ItemKind, kind_raw)
        item_id_raw = item.get("id")
        if not isinstance(item_id_raw, int):
            raise ValueError("Clipboard item ID must be an integer.")
        item_id = item_id_raw
    ...
    created: list[int] = []
    for item in clipboard_items:
        kind = cast(workspace_browser.ItemKind, item["kind"])
        item_id = int(item["id"])
        if kind == "folder":
            created.append(_copy_item(storage_root, project_id, kind, item_id, target_folder_id))
        elif kind == "note":
            created.append(workspace_browser.create_note(project_id, _unique_name(project_id, target_folder_id, str(item["name"])), str(item.get("content", "")), target_folder_id, metadata=cast(dict[str, Any], item.get("metadata") or {})))
        else:
            created.append(workspace_browser.create_file(storage_root, project_id, _unique_name(project_id, target_folder_id, str(item["name"])), bytes(item["data"]), None if item.get("mime_type") is None else str(item["mime_type"]), target_folder_id))
    return created
