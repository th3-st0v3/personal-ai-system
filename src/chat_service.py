...
def list_chats(connection: sqlite3.Connection, project_id: int | None = None) -> list[dict[str, object]]:
    rows = connection.execute(
        "SELECT id, project_id, title, pinned, created_at, updated_at FROM chats "
        "WHERE project_id IS ? ORDER BY pinned DESC, updated_at DESC, id DESC",
        (project_id,),
    ).fetchall()
...
def get_chat(connection: sqlite3.Connection, chat_id: int) -> dict[str, object]:
    ...
    return {
        "id": row[0], "project_id": row[1], "title": row[2], "pinned": bool(row[3]),
        "created_at": row[4], "updated_at": row[5],
        "messages": [{"id": m[0], "role": m[1], "content": m[2], "created_at": m[3]} for m in messages],
    }
...
def _openrouter(messages: list[dict[str, object]], project_id: int | None, model: str | None = None) -> tuple[str, list[dict[str, object]]]:
    ...
    for _ in range(_MAX_TOOL_ROUNDS):
        ...
        request = urllib.request.Request(
            ...
            headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json", "HTTP-Referer": "http://localhost", "X-Title": "Personal AI System"},
            method="POST",
        )
        ...
        if not calls:
            return message.get("content") or "", events
        ...
        for call in calls:
            ...
            try:
                ...
                if tool_name == "search_project_sources" and isinstance(result.get("results"), list):
                    event["sources"] = [
                        {
...
def _local_answer(content: str) -> str:
    ...
    if any(word in tokens for word in ("model", "simulate", "simulation", "calculate", "calculator")):
        ...
        if plan["simulations"]:
            lines += ["Suggested simulations:"] + [f"- {item['name']} ({item['key']})" for item in plan["simulations"]]
        lines += ["Assumptions:"] + [f"- {item}" for item in plan["assumptions"]]
        if plan["open_questions"]:
            lines += ["Open questions:"] + [f"- {item}" for item in plan["open_questions"]]
...
def respond(connection: sqlite3.Connection, chat_id: int, content: str, *, model: str | None = None, mode: str = "auto") -> dict[str, object]:
    ...
    messages = [{"role": m["role"], "content": m["content"]} for m in get_chat(connection, chat_id)["messages"]]
    events: list[dict[str, object]] = []
    try:
        if mode == "local":
