...
def _note(connection: sqlite3.Connection, note_id: int) -> BrowserItem | None:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    return BrowserItem("note", int(row[0]), int(row[1]), None if row[2] is None else int(row[2]), str(row[3]), "text/markdown", str(row[4]), None, str(row[5]), str(row[6]), cast(dict[str, Any], metadata))
...
def create_note(project_id: int, title: str, content: str = "", folder_id: int | None = None, *, metadata: dict[str, Any] | None = None) -> int:
    ...
    try:
        ...
        if folder_id is not None:
            workspace_storage._require_folder_in_project(c, folder_id, project_id)
        cursor = c.execute("INSERT INTO workspace_notes(project_id,folder_id,title,content,metadata) VALUES(?,?,?,?,?)", (project_id, folder_id, title, content, json.dumps(metadata or {}, sort_keys=True)))
        c.commit()
        return _lastrowid(cursor, "note")
    finally:
        c.close()
...
def update_note(note_id: int, *, title: str | None = None, content: str | None = None, metadata: dict[str, Any] | None = None) -> BrowserItem:
    ...
    try:
        ...
        if updates:
            c.execute(f"UPDATE workspace_notes SET {', '.join(updates)}, updated_at=datetime('now') WHERE id=?", (*params, note_id))
            c.commit()
        updated = _note(c, note_id)
        if updated is None:
            raise RuntimeError(f"Note {note_id} disappeared during update.")
        return updated
    finally:
        c.close()
...
def move_note(note_id: int, folder_id: int | None) -> None:
    ...
    try:
        ...
        if note is None:
            raise ValueError(f"No note found with ID {note_id}.")
        if folder_id is not None:
            workspace_storage._require_folder_in_project(c, folder_id, note.project_id)
        c.execute("UPDATE workspace_notes SET folder_id=?, updated_at=datetime('now') WHERE id=?", (folder_id, note_id))
        c.commit()
    finally:
        c.close()
...
def delete_note(note_id: int) -> None:
    c = _connection()
    try:
        if _note(c, note_id) is None:
            raise ValueError(f"No note found with ID {note_id}.")
        c.execute("DELETE FROM workspace_notes WHERE id=?", (note_id,))
        c.commit()
    finally:
        c.close()
...
def list_children(project_id: int, folder_id: int | None = None, *, sort: str = "a_z") -> list[BrowserItem]:
    ...
    try:
        ...
        field = SORT_OPTIONS[sort]
        if field == "name":
            return sorted(items, key=lambda x: (x.name.casefold(), x.kind, x.id), reverse=sort == "z_a")
...
def list_project_items(project_id: int, *, recursive: bool = True, sort: str = "a_z") -> list[BrowserItem]:
    ...
    while queue:
        ...
        if recursive:
            queue.extend(i.id for i in children if i.kind == "folder")
    return result
...
def create_file(storage_root, project_id: int, name: str, data: bytes, mime_type: str | None = None, folder_id: int | None = None) -> int:
    ...
    if created_id is None:
        raise RuntimeError("Database did not return a new file ID.")
...
def _folder_descendants(project_id: int, folder_id: int) -> list[int]:
    c = _connection()
    try:
        return [int(r[0]) for r in c.execute("WITH RECURSIVE tree(id) AS (SELECT id FROM folders WHERE id=? AND project_id=? UNION ALL SELECT f.id FROM folders f JOIN tree t ON f.parent_folder_id=t.id) SELECT id FROM tree", (folder_id, project_id)).fetchall()]
    finally:
        c.close()
...
def _delete_folder_tree(storage_root, project_id: int, folder_id: int) -> None:
    ...
    try:
        ...
        note_ids = [int(r[0]) for r in c.execute(f"SELECT id FROM workspace_notes WHERE project_id=? AND folder_id IN ({placeholders})", (project_id, *descendants)).fetchall()]
    finally:
        c.close()
...
def delete_selection(storage_root, project_id: int, selection: Iterable[tuple[ItemKind, int]]) -> int:
    selected = {(cast(ItemKind, kind), int(item_id)) for kind, item_id in selection}
    if any(not _exists(cast(ItemKind, kind), item_id) for kind, item_id in selected):
        raise ValueError("Selection contains a missing workspace item.")
    if any(_item_project(cast(ItemKind, kind), item_id) != project_id for kind, item_id in selected):
    ...
    deleted = 0
    for fid in roots:
        _delete_folder_tree(storage_root, project_id, fid)
        deleted += 1
    for kind, item_id in selected:
        ...
        if kind == "file":
            ...
            if row is None:
                continue
            if any(_folder_contains(project_id, None if row[2] is None else int(row[2]), fid) for fid in roots):
                continue
            ...
            if note is None:
                continue
            if any(_folder_contains(project_id, note.parent_id, fid) for fid in roots):
                continue
            delete_note(item_id)
        deleted += 1
    return deleted
...
def _exists(kind: ItemKind, item_id: int) -> bool:
    if kind == "folder":
        return workspace_storage.get_folder(item_id) is not None
    if kind == "file":
        return workspace_storage.get_file(item_id) is not None
...
def _item_project(kind: ItemKind, item_id: int) -> int:
    if kind == "folder":
        ...
        if row is None:
            raise ValueError(f"No folder found with ID {item_id}.")
        return int(row[1])
    if kind == "file":
        row = workspace_storage.get_file(item_id)
        if row is None:
            raise ValueError(f"No file found with ID {item_id}.")
        return int(row[1])
    note = get_note(item_id)
    if note is None:
        raise ValueError(f"No note found with ID {item_id}.")
...
def get_breadcrumbs(kind: ItemKind, item_id: int) -> list[tuple[str, int, str]]:
    ...
    result_folders: list[tuple[str, int, str]] = []
    while folder_id is not None:
        folder = workspace_storage.get_folder(folder_id)
        if folder is None or int(folder[1]) != project_id:
            break
        result_folders.append(("folder", int(folder[0]), str(folder[3])))
        folder_id = None if folder[2] is None else int(folder[2])
    result.extend(reversed(result_folders))
    return result
