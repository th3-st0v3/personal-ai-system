    ...
    def _manifest(self, user: dict[str, object] | None = None) -> dict[str, object]:
        ...
        return {
            ...
            "workspace": {
                ...
                "multi_selection_actions": ["open", "copy", "move", "archive", "invalidate", "delete", "properties"],
            },
    ...
    def request(self, method: str, target: str, body: bytes = b"", environ: dict[str, object] | None = None) -> tuple[int, list[tuple[str, str]], bytes]:
        try:
            ...
            if method == "GET" and path == "/api/manifest":
                return self._json(200, self._manifest(user))

            if method == "POST" and path == "/api/auth/signup":
                connection = db.get_connection()
                try:
                    created = auth_service.signup(connection, data["email"], data["password"], data.get("display_name"))
                    session, user = auth_service.login(connection, data["email"], data["password"])
                finally:
                    connection.close()
            ...
            if method == "POST" and path == "/api/auth/logout":
                ...
                return self._json(200, {"logged_out": True}, [("Set-Cookie", "pas_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0")])
            ...
            if method == "GET" and path == "/api/simulations":
                return self._json(200, [{"key": s.key, "name": s.name, "discipline": s.discipline, "description": s.description, "parameters": list(s.parameters)} for s in simulation_library.list_simulations()])
            ...
            if method == "POST" and path == "/api/chats":
                ...
                try:
                    chat_service.initialize(connection)
                    chat_id = chat_service.create_chat(connection, data.get("project_id"), data.get("title", "New chat"))
                finally:
                    connection.close()
            ...
            if method == "GET" and path == "/api/chats":
                ...
                connection = db.get_connection()
                try:
                    return self._json(200, chat_service.list_chats(connection, project_id))
                finally:
                    connection.close()
            if method == "GET" and path.startswith("/api/chats/") and path.count("/") == 3:
                connection = db.get_connection()
                try:
                    return self._json(200, chat_service.get_chat(connection, int(path.rsplit("/", 1)[-1])))
                finally:
                    connection.close()
            if method == "POST" and path.startswith("/api/chats/") and path.endswith("/messages"):
                ...
                connection = db.get_connection()
                try:
                    return self._json(200, chat_service.respond(connection, chat_id, data["content"], model=data.get("model")))
                finally:
                    connection.close()
            ...
            if len(parts) >= 4 and parts[1:3] == ["api", "projects"]:
                ...
                if method == "GET" and len(parts) == 4:
                    connection = db.get_connection()
                    try:
                        return self._json(200, project_service.get_project(connection, project_id))
                    finally:
                        connection.close()
                if method == "PATCH" and len(parts) == 4:
                    connection = db.get_connection()
                    try:
                        return self._json(200, project_service.update_project(connection, project_id, name=data.get("name"), description=data.get("description")))
                    finally:
                        connection.close()
                ...
                if method == "DELETE" and len(parts) == 5 and parts[4] == "notes":
                    note_id = int(data["id"])
                    self.workspace.delete_note(note_id, project_id=project_id)
                    return self._json(200, {"deleted": True})
                ...
                if method == "PUT" and len(parts) == 5 and parts[4] == "files":
                    ...
                    return self._json(200, {"id": file["id"], "name": file["name"], "size_bytes": file["size_bytes"], "mime_type": file["mime_type"]})
