...
def as_list(value: object) -> list[dict[str, object]]:
    if not isinstance(value, list): raise AssertionError(f"expected list, got {value!r}")
...
class TestIngestionPolicy(unittest.TestCase):
    ...
    def test_text_ingestion_is_versioned_and_searchable(self):
        ...
        try:
            source = ingestion_service.ingest_text(connection, self.project_id, "test.md", "Pressure requirement: 5 MPa.\nUse steel.", version="1", chunk_chars=100)
    ...
    def test_policy_defaults_and_explicit_grants(self):
        ...
        try:
            ...
            with self.assertRaises(PermissionError): policy.require(connection, "local", "execute_code")
    ...
    def test_invalid_actions_are_rejected(self):
        connection = db.get_connection()
        try:
            with self.assertRaises(ValueError): policy.validate_action("sudo")
        finally: connection.close()
