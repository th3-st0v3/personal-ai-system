...
class EngineeringWebApplication:
    ...
    def request(self, method: str, target: str, body: bytes = b"") -> tuple[int, list[tuple[str, str]], bytes]:
        try:
            ...
            if len(parts) < 3 or parts[:2] != ["api", "engineering", "projects"]:
            ...
            if resource == "requirements" and len(parts) == 5 and method == "PATCH":
                requirement_id = int(parts[4])
            ...
            if method == "POST" and resource == "sources":
                ...
                if file_id is not None:
                    record = self.engineering._source if False else None
                    file = self._workspace_file(project_id, int(file_id))
                    if file is None:
            ...
            if resource == "requirements" and len(parts) == 6 and parts[5] == "evidence":
                requirement_id = int(parts[4])
                self._require_requirement(project_id, requirement_id)
                if method == "GET":
            ...
            if resource == "evidence" and len(parts) == 5 and method == "POST" and parts[4] == "invalidate":
