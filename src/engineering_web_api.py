...
class EngineeringWebApplication:
    ...
    def request(self, method: str, target: str, body: bytes = b"") -> tuple[int, list[tuple[str, str]], bytes]:
        try:
            ...
            if resource == "requirements" and len(parts) == 6 and method == "PATCH":
                requirement_id=int(parts[5]); self._require_requirement(project_id,requirement_id); self.engineering.update_requirement(requirement_id,identifier=data.get("identifier"),title=data.get("title"),acceptance_criteria=data.get("acceptance_criteria"),priority=data.get("priority"),status=data.get("status")); return self._json(200,next(item for item in self.engineering.list_requirements(project_id) if item["id"]==requirement_id))
            if method == "GET" and len(parts) == 5 and resource == "sources": return self._json(200,self.engineering.list_sources(project_id))
            ...
            if method == "GET" and len(parts) == 5 and resource == "report":
                requirements,evidence_by_requirement=self._requirements_with_evidence(project_id); return self._json(200,engineering_plans.build_weekly_report(requirements,self.engineering.list_sources(project_id),self.engineering.list_decisions(project_id),evidence_by_requirement))
            if resource == "requirements" and len(parts) == 7 and parts[6] == "evidence":
                requirement_id=int(parts[5]); self._require_requirement(project_id,requirement_id)
                if method=="GET":
                    connection=db.get_connection()
                ...
                if method=="POST":
                    ...
                    evidence_id=self.engineering.create_evidence(requirement_id,data["result"],data["supports_status"],source=data.get("source"),location=data.get("location"),calculation_record_id=data.get("calculation_record_id"),source_id=source_id,classification=data.get("classification"),description=data.get("description"),evidence_type=data.get("evidence_type"))
                    return self._json(201,{"id":evidence_id})
            if resource == "evidence" and len(parts)==6 and method=="POST" and parts[5]=="invalidate":
                evidence_id=int(data["id"]); self._require_evidence(project_id,evidence_id); self.engineering.invalidate_evidence(evidence_id,data["reason"]); return self._json(200,{"invalidated":True})
            if method=="GET" and len(parts)==5 and resource=="decisions": return self._json(200,self.engineering.list_decisions(project_id))
            if method=="POST" and len(parts)==5 and resource=="decisions":
                requirement_id=data.get("requirement_id"); design_case_id=data.get("design_case_id")
                if requirement_id is not None:self._require_requirement(project_id,int(requirement_id))
                if design_case_id is not None:self._require_design_case(project_id,int(design_case_id))
        ...
        except (KeyError,ValueError,TypeError,json.JSONDecodeError) as exc:return self._json(400,{"error":str(exc) or "Invalid request"})
    ...
    def _workspace_file(project_id:int,file_id:int):
        ...
        row=get_file(file_id); return row if row is not None and row[1]==project_id else None
    ...
    def __call__(self,environ,start_response):
        try:
            ...
            if length>self.MAX_REQUEST_BODY_BYTES:status,headers,payload=self._json(413,{"error":"Request body too large."})
                ...
                body=environ["wsgi.input"].read(length) if length else b"";status,headers,payload=self.request(environ.get("REQUEST_METHOD","GET"),target,body)
...
def create_engineering_app(engineering:EngineeringApplication|None=None)->EngineeringWebApplication:return EngineeringWebApplication(engineering)
