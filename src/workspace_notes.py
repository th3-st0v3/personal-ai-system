...
Notes are real project resources. They may live at the project root, inside a
folder, or inside another note. The service keeps those relationships inside
SQLite while exposing a small application-friendly API for the future web UI.
"""
...
SORTS = {
    "name_asc": "content COLLATE NOCASE ASC, id ASC",
    "name_desc": "content COLLATE NOCASE DESC, id DESC",
...
def _would_create_cycle(connection, note_id, parent_note_id):
    ...
    while current is not None:
        if current == note_id:
            return True
        row = connection.execute(
            "SELECT parent_note_id FROM notes WHERE id = ?", (current,)
        ).fetchone()
...
def create_note(project_id, content, *, folder_id=None, parent_note_id=None, source="user"):
    ...
    try:
        ...
        cursor = connection.execute(
            "INSERT INTO notes (project_id, folder_id, parent_note_id, content, source, updated_at) "
            "VALUES (?, ?, ?, ?, ?, datetime('now'))",
            (project_id, folder_id, parent_note_id, content, source),
        )
        connection.commit()
...
def get_note(note_id):
    ...
    try:
        row = connection.execute(
            "SELECT id, project_id, folder_id, parent_note_id, content, source, "
...
def move_note(note_id, *, folder_id=None, parent_note_id=None):
    ...
    try:
        ...
        connection.execute(
            "UPDATE notes SET folder_id = ?, parent_note_id = ?, updated_at = datetime('now') "
            "WHERE id = ?",
...
def update_note(note_id, *, content=None, source=None):
    connection = _connection()
    try:
...
def delete_note(note_id):
    ...
    try:
        ...
        child = connection.execute(
            "SELECT 1 FROM notes WHERE parent_note_id = ? LIMIT 1", (note_id,)
        ).fetchone()
...
def delete_notes(note_ids):
    ...
    try:
        ...
        connection.execute(
            "DELETE FROM notes WHERE id IN (" + placeholders + ")", note_ids
        )
        connection.commit()
    finally:
        connection.close()
